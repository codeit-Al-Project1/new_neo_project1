...
def compute_map(targets, predictions, iou_threshold=0.5):
    """Mean Average Precision (mAP) 계산"""
    ap_list = []
    
    # 각 클래스별로 Precision과 Recall을 계산하고 AP를 구합니다.
    for i in range(len(targets)):
        precision, recall, tp, fp, fn = compute_precision_recall(targets[i], predictions[i], iou_threshold)
